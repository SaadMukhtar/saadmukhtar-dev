...
export const conceptFlashcards: ConceptFlashcard[] = [
  ...
  // Missing Core Concepts
  {
    id: "consistent-hashing",
    term: "Consistent hashing",
    prompt: "What is consistent hashing, and what problem does it solve that plain modulo hashing doesn't?",
    keyPoint:
      "Arrange both servers and keys on a virtual ring (hash space). Each key is owned by the next server clockwise. Adding or removing a server moves only ~1/N of keys, not all of them. Fixes the cache-stampede-on-resize problem plain hash-mod-N causes.",
    answer:
      "Plain mod-N sharding (key % N → server) is simple but disastrous when N changes: adding or removing a server remaps almost every key, causing cache stampedes and massive data movement. Consistent hashing maps both servers and keys onto a virtual ring (hash space, e.g. 0 to 2^32). Each key is owned by the next server clockwise. When you add a server, only the keys between its position and the previous owner move; when you remove one, only its keys shift to the next server. Uses virtual nodes (each physical server gets many ring positions) to smooth load imbalance. It's the foundation of DynamoDB, Cassandra, memcached clusters, and most modern distributed caches.",
    category: "Distributed Systems & Consistency",
  },
  {
    id: "numbers-to-know",
  ...
  },
  {
    id: "vector-clocks",
    term: "Vector clocks",
    prompt: "What is a vector clock, and what problem does it solve in distributed systems?",
    keyPoint:
      "A vector of counters, one per node, that lets you detect concurrent vs causally-related events across a distributed system. If A's vector strictly dominates B's, A happened after B; if neither dominates, they're concurrent (conflicting).",
    answer:
      "Physical clocks (wall time) drift across nodes and can't be trusted for ordering events in distributed systems. Vector clocks assign each node a counter that increments on local events and is included in every message. A node receiving a message merges the sender's vector with its own (element-wise max, then increment). To compare two events: if vector A ≥ vector B on every element, A happened after B (or B is a subset). If neither dominates, the events are concurrent — meaning they may conflict and require resolution (last-write-wins, semantic merge, CRDT). Used in Dynamo-style databases for conflict detection, in causal broadcast, and in some collaboration tools (like Automerge).",
    category: "Distributed Systems & Consistency",
  },
  {
    id: "quorum-reads-writes",
    term: "Quorum reads and writes (R + W > N)",
    prompt: "What are quorum reads/writes, and why is R + W > N the magic condition for strong consistency?",
    keyPoint:
      "With N replicas, W = write quorum (writes must ack from W nodes), R = read quorum (reads query R nodes). If R + W > N, every read overlaps with every prior write on at least one node, so you always see the latest value. Common: N=3, W=2, R=2.",
    answer:
      "In a replicated key-value store with N replicas, W is the number of replicas that must acknowledge a write before it's considered successful, R is the number queried on a read. If R + W > N, the read set and the write set must overlap by at least one replica — that replica has the latest write, so reads always see it (strong consistency). If R + W ≤ N, reads may miss recent writes (eventual consistency). The tradeoff is latency and availability: higher W = slower writes but stronger reads; higher R = slower reads. Classic Dynamo tuning: N=3, W=2, R=2 gives strong consistency and tolerates 1 node down for either read or write. Cassandra exposes this as a per-query knob (LOCAL_QUORUM, ONE, ALL).",
    category: "Distributed Systems & Consistency",
  },
  {
    id: "gossip-protocols",
    term: "Gossip protocols",
    prompt: "What's a gossip protocol, and when do you use it?",
    keyPoint:
      "Each node periodically picks a random peer and exchanges state. Membership and cluster metadata propagate in O(log N) rounds without a central coordinator. Used by Cassandra, Consul, Serf for cluster membership and failure detection.",
    answer:
      "Gossip (or epidemic) protocols spread information across a distributed system by having each node periodically pick a random peer and exchange state — like a rumor spreading through a crowd. Information reaches all N nodes in O(log N) rounds with high probability, without any central coordinator or all-pairs communication. Each message is small and the load is evenly distributed. Used for cluster membership (which nodes are alive?), failure detection (haven't heard from a node in K rounds → mark suspect), and metadata dissemination (schema versions, topology). Cassandra, Consul, and Serf all use gossip for cluster state. Not suitable for consistent state (use Raft/Paxos for that); gossip is eventually consistent by design.",
    category: "Distributed Systems & Consistency",
  },
